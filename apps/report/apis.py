...
unscramble_rule = dict(
    transmission=[
        ...
        ("{account_max} == {post_max}", "从账号分布来看，{account_max}账号数量最多（{account_count}个），发布的帖子数也最多（{account_post_count}篇）\n"),
        ("{account_max} != {post_max}", "从账号分布来看，{account_max}账号数量最多（{account_count}个），而{account_post_max}账号发布帖子数最多（{account_post_count}篇）\n"),
...
def get_unscramble(data):
    ...
    account_max_df = pd.DataFrame(data["spread_overview"]["account"])
    account_max = account_max_df.drop_duplicates(subset=["account", "platform"]).groupby(["user_type"], as_index=False).agg({"account": pd.Series.nunique}).sort_values("account", ascending=False).iloc[0]
    post_max = account_max_df.groupby(["user_type"], as_index=False).agg({"post_count": pd.Series.sum}).sort_values("post_count", ascending=False).iloc[0]

    spread_efficiency = data["spread_efficiency"]["platform_cumulative"]
    cb_platform_max = max(spread_efficiency, lambda x: x["breadth"])
    hd_platform_max = max(spread_efficiency, lambda x: x["interaction"])

    account_cumulative = data["spread_efficiency"]["account_cumulative"]
    cb_account_max = max(account_cumulative, lambda x: x["breadth"])
    hd_account_max = max(account_cumulative, lambda x: x["interaction"])

    activity_cumulative = data["spread_efficiency"]["activity_cumulative"]
    cb_activity_max = max(activity_cumulative, lambda x: x["breadth"])
    hd_activity_max = max(activity_cumulative, lambda x: x["interaction"])

    platform_cumulative = data["spread_efficiency"]["platform_cumulative"]
    source_max = max(platform_cumulative, lambda x: x["value"])

    user_type_composition = data["spread_efficiency"]["user_type_composition"]
    account_comment_max = max(user_type_composition, lambda x: x["value"])

    activity_ugc_max = data["spread_effectiveness"]["activity_ugc_in_activity_composition"]
    ...
    param = dict(
        ...
        source_ratio=float(source_max["value"] * 100.0 / sum([x["value"] for x in platform_cumulative]), 2),

        account_comment_max=source_max["name"],
        account_comment_ratio=float(account_comment_max["value"] * 100.0 / sum([x["value"] for x in user_type_composition]), 2),
        ...
        activitys_brand_ugc_ratio=float(data["spread_effectiveness"]["activity_brand_ugc_count"] * 100.0 / data["spread_effectiveness"]["ugc_in_activity_count"], 2),
        ...
        brand_ugc_diff_ratio=abs(float(data["spread_effectiveness"]["ugc_in_activity_count"] * 100.0 / data["spread_effectiveness"]["predict"], 2) - 1),

        brand_attention=data["brand_concern"]["activity"],
        brand_attention_year=data["brand_concern"]["annual"],
        brand_attention_ratio=abs(float(data["brand_concern"]["activity"] - data["brand_concern"]["annual"] * 100, 2)),

        sales_point_cognitive=data["tags_concern"]["activity"],
        sales_point_cognitive_year=data["tags_concern"]["annual"],
        sales_point="sales_point",
        sales_point_cognitive_ratio=abs(float(data["tags_concern"]["activity"] - data["tags_concern"]["annual"] * 100, 2)),
